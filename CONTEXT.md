...
- **Gas cut** — the fee BYOS declares on every fulfillment it bids, sized at exactly the settlement's estimated gas cost and denominated in the **order's sell token**. It is *kept*, not reimbursed: the settlement pays the user out at prices that leave the cut in the `GPv2Settlement` buffers, and CoW's weekly accounting values it in native token later. Always on, no rate to configure. This is the only fee BYOS charges — the older "BYOS fee" (a percentage of `sellAmount`) never shipped, and CoW's own **protocol fee** and the contracts docs' **network fee** are different things applied by the driver, not by us ([ADR-0002](docs/adr/0002-solver-engine.md) §Fee mechanism).
...
1. **Solver engine** (`crates/byos`) — answers the standard CoW driver's `/solve` from the proposal store; internal `surplus - gas` pre-ranking; single best proposal per order UID; fat-Trampoline settlement crafting ([ADR-0002](docs/adr/0002-solver-engine.md)).
