...
**ADRs must never contradict each other.** When a new ADR supersedes part of an older one, the same change edits the older ADR to agree with the new one: delete the superseded content and leave a short pointer to the ADR that now owns the topic. Do not keep stale text under a "superseded" banner — ADRs stay light, and history lives in git, not in annotated dead prose. "Append-only" means a changed decision gets a new ADR rather than a silent rewrite of the old one; it does not mean old ADRs keep asserting things that are no longer true. An ADR still in `proposed` status may be rewritten in place, with a note recording the rewrite.
