...
The service needs operational configuration (ports, RPC URLs, log filters) and behavioral configuration (chain, contract addresses, rate limits, simulation interval). Services splits these across clap CLI args and TOML files; we adopt that split.
...
BYOS-specific config concerns the TOML must cover from day one: chain id, `GPv2Settlement` / TrampolineFactory / Escrow addresses (GPv2Settlement is the `--settlement-address` CLI arg), `c_l` fallback value ([ADR-0003](0003-slash-attribution-flow.md)), rate-limit tiers ([ADR-0001](0001-proposal-api.md)), simulation interval, public and internal listener addresses, audit-trail path/retention.
