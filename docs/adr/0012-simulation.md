...
- **Hooked orders are supported when `--hooks-trampoline` is configured** (COW-1243). Pre/post hooks from `fullAppData.metadata.hooks` are parsed, stored on the proposal, and encoded as `HooksTrampoline.execute()` calls in both the simulation and `/solve` solutions. Bridging orders remain rejected.
