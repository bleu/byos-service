...
- **Hooked orders are supported** (COW-1243). Pre/post hooks from `fullAppData.metadata.hooks` are pre-encoded by the orderbook as `HooksTrampoline.execute()` calls and included in simulation for accurate gas estimates. The `/solve` response does NOT include hooks — the driver appends hooks itself. This includes partially fillable orders with hooks: the envelope accepts both features, and settlement encoding handles `executedAmount` (partial fill fraction) and hook interactions (`interactions[0]`/`[2]`) independently. **Known trade-off:** gas is overestimated for non-first fills because the order cache always includes pre-interactions; fixing this would require tracking `executedAmount` and invalidating the cache. Bridging orders remain rejected.
